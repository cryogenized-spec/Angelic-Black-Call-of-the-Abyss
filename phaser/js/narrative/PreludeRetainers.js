...
(function(){
  ...
  function skull(g,variant){
    ...
    // Dark outer skull silhouette.
    poly(g,[
      {x:21+dx,y:8},{x:42+dx,y:8},{x:48+dx,y:14},{x:48+dx,y:28},
      {x:43+dx,y:35},{x:38+dx,y:39},{x:25+dx,y:39},{x:19+dx,y:34},{x:16+dx,y:27},{x:16+dx,y:15}
    ],P.ink);
    // Bone shell with blocky planes.
    poly(g,[
      {x:22+dx,y:10},{x:40+dx,y:10},{x:45+dx,y:15},{x:45+dx,y:27},
      {x:41+dx,y:33},{x:36+dx,y:36},{x:26+dx,y:36},{x:21+dx,y:32},{x:19+dx,y:25},{x:19+dx,y:15}
    ],P.bone);
  ...
  function armourA(g){
    // Retainer A: broader royal guard, reinforced shoulder plate and scabbard.
  ...
  function weaponA(g){
    // Curved sword on the right side.
  ...
  function weaponB(g){
    // Ceremonial polearm on the left side.
  ...
  function renderRetainer(variant){
    const g=new Phaser.GameObjects.Graphics({scene:null});
    // Scene is injected by caller; this function is replaced by generateTexture's graphics context.
    return g;
  }
  ...
  function generate(scene,key,variant){
    ...
    // Sparse transparent canvas: no background, so the sprite reads cleanly in the tomb.
    skull(g,variant);
    if(variant===1){armourA(g);weaponA(g);}else{armourB(g);weaponB(g);}
    // Pixel-art ground/contact shadow baked very subtly into feet.
