/* Angelic Black — Phaser M7 progression / death / level-up foundation. */
class ProgressionSystem {
  ...
  awardXp(amount){
    ...
    this.xp+=amount;
    if(this.xp>=this.xpNeed){this.xp-=this.xpNeed;this.level++;this.xpNeed=Math.floor(this.xpNeed*1.28+20);this.openLevelUp();}
  }
  ...
  chooseLevelUp(index){
    ...
    const o=this.levelOverlay;Object.values(o).forEach(obj=>obj.setVisible(false));this.scene.showBanner(`${choice.label} CHOSEN`);
  }
  onQueenDefeated(){
    if(this.state==='death')return;this.state='death';this.continues=Math.max(0,this.continues-1);
  ...
  respawn(){
    ...
    this.scene.time.delayedCall(800,()=>{if(this.queen.active)this.queen.invulnerable=false;});this.scene.clearEnemiesForRespawn?.();Object.values(this.deathOverlay).forEach(obj=>obj.setVisible(false));this.scene.showBanner('THE QUEEN RISES AGAIN');this.scene.waves.resetToWaveOne?.();
  }
  update(){
    if(this.state==='level-up'){if(Phaser.Input.Keyboard.JustDown(this.scene.levelOneKey))this.chooseLevelUp(0);if(Phaser.Input.Keyboard.JustDown(this.scene.levelTwoKey))this.chooseLevelUp(1);return;}
    if(this.state==='death'&&Phaser.Input.Keyboard.JustDown(this.scene.resetKey)){if(this.continues>0)this.respawn();else this.scene.restart();}
  }
}
