...
(function(){
  ...
  const actions={
    ...
    walk:{frames:3,fps:12,repeat:-1,texture:'queen-idle'},
    jump:{frames:3,fps:10,repeat:0,texture:'queen-idle'},
    cast:{frames:3,fps:10,repeat:0,texture:'queen-idle'},
    special:{frames:3,fps:9,repeat:0,texture:'queen-idle'},
  ...
  window.ANGELIC_QUEEN_ASSETS={
    ...
    defineAnimations(scene){
      Object.entries(actions).forEach(([action,spec])=>{
        ...
        const textureKey=spec.texture||key;
        const hasTexture=scene.textures.exists(textureKey);
        const fallback=scene.textures.exists('queen-idle')?'queen-idle':'queen-fallback';
        const source=hasTexture?textureKey:fallback;
        const count=hasTexture?spec.frames:1;
        const frames=scene.anims.generateFrameNumbers(source,{start:0,end:Math.max(0,count-1)});
        scene.anims.create({key,frames,frameRate:spec.fps,repeat:spec.repeat,skipMissedFrames:true});
      });
      const fall='queen-fall';if(scene.anims.exists(fall))scene.anims.remove(fall);
      const jumpTex=scene.textures.exists('queen-idle')?'queen-idle':'queen-fallback';
      scene.anims.create({key:fall,frames:[{key:jumpTex,frame:2}],frameRate:1,repeat:-1});
    },
