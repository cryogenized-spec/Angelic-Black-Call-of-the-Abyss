...
(function(root){
  ...
  var ART={
    ...
    comic:[base+'comic/comic-01-tomb-sealed.png',base+'comic/comic-02-seal-decays.png',base+'comic/comic-03-queen-awakens.png',base+'comic/comic-04-first-tomb.png'],
  ...
  var LEGACY={cover:POSTER_URL,comic:COMIC_PAGES.slice(),malachar:'https://image.qwenlm.ai/public_source/de086523-4053-4904-8d82-f5f98bd17fc5/1061cc221-1c1e-4ad5-9ac2-8629b9f9dd3e.png',aftermath:AFTERMATH_IMG,witch:WITCH_IMG};
  root.__ANGELIC_BLACK_ART__=ART;

  function localImage(path,onReady){var img=new Image();img.onload=function(){if(onReady)onReady(img);};img.onerror=function(){};img.src=path;return img;}
  function useLocalImage(target,local,legacy){
    if(!target)return;
    var original=target.src||legacy; target.src=original;
    var img=new Image(); img.onload=function(){target.src=local;}; img.onerror=function(){target.src=original;}; img.src=local;
  }

  /* Title art: legacy remains visible until the authored local PNG exists. */
  coverArtEl.style.backgroundImage="url('"+LEGACY.cover+"')";
  localImage(ART.cover,function(){coverArtEl.style.backgroundImage="url('"+ART.cover+"')";});

  /* Comic art: original remote pages remain the fallback during upload. */
  var comicLocal=[];
  for(var i=0;i<ART.comic.length;i++){
    (function(index){
      comicLocal[index]=new Image();
      comicLocal[index].onload=function(){if(typeof comicIdx==='number'&&comicIdx===index&&comicImg)comicImg.src=ART.comic[index];};
  ...
  /* Existing cutscene image objects have already been created by module 10. */
  ...
  var oldGuardImg=localImage(ART.oldGuard,function(){authoredReady.oldGuard=true;});
  var malacharFallsImg=localImage(ART.malacharFalls,function(){authoredReady.malacharFalls=true;});
  ...
  if(typeof root.introRiseCS==='function'){
    ...
    root.introRiseCS=function(){
      if(!authoredReady.oldGuard){root.__AB_ORIGINAL_INTRO_RISE_CS__();return;}
      playCS([
        {k:'still',v:1,img:oldGuardImg},
        {k:'wait',d:0.55},
        {k:'say',who:'THE NECRO QUEEN',text:'Rise.'},
        {k:'fn',run:function(){shakeT=0.3;sfx.slam();sfx.rise();minions.push(makeRetinue('nameA',player.x-46));minions.push(makeRetinue('nameB',player.x+46));dirtBurst(player.x-46);dirtBurst(player.x+46);}},
        {k:'wait',d:1.5},
  ...
  if(typeof root.bossDefeatCS==='function'){
    ...
    root.bossDefeatCS=function(f){
      if(!authoredReady.malacharFalls){root.__AB_ORIGINAL_BOSS_DEFEAT_CS__(f);return;}
      playCS([
        ...
        {k:'still',v:0},{k:'fn',run:function(){player.castFx=0.5;}},
        {k:'say',who:'THE NECRO QUEEN',text:'CORPSE BOMB!'},{k:'fn',run:function(){corpseBomb(f);}},
        {k:'shake',amp:0.6,d:0.5},{k:'wait',d:1.6},{k:'fn',run:function(){inv.signet=1;}},
        {k:'say',who:'THE NECRO QUEEN',text:'...My seal.'},{k:'black',d:0.7},
        {k:'still',v:1,img:malacharFallsImg},{k:'unblack',d:0.9},{k:'wait',d:0.6},
        ...
        {k:'black',d:0.8},{k:'still',v:1,img:roadImg},{k:'unblack',d:0.9},{k:'wait',d:1.4},
        {k:'crows',mode:'in',d:2.4},{k:'black',d:0.3},{k:'still',v:1,img:witchImg},{k:'unblack',d:0.5},
