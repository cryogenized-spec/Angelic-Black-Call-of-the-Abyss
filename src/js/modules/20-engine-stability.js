...
(function(root){
  ...
  if(!runtime)return;
  var stability={version:'pass-5',frames:0,lastFrameMs:0,maxFrameMs:0,droppedFrames:0,validationFailures:0,hiddenSince:0,lastValidation:0,frameStart:0,expectedFrameMs:1000/60,debug:(new URLSearchParams(root.location.search)).get('debug')==='1',panel:null,paused:false,scanlines:null};
  ...
  function updatePanel(){
    ...
    stability.panel.textContent='AB ENGINE 3.1 | '+stability.lastFrameMs.toFixed(1)+'ms | max '+stability.maxFrameMs.toFixed(1)+'ms | entities '+count+' | dropped '+stability.droppedFrames+' | validation '+stability.validationFailures+(stability.paused?' | PAUSED':'');
  }
  function makeScanlines(){
    var c=document.createElement('canvas');
    c.width=540;c.height=675;
    var g=c.getContext('2d');
    g.fillStyle='rgba(5,2,10,0.15)';
    for(var y=0;y<675;y+=3)g.fillRect(0,y,540,1);
    return c;
  }
  stability.scanlines=makeScanlines();
  stability.drawScanlines=function(g){g.drawImage(stability.scanlines,0,0);};
  stability.beginFrame=function(){stability.frameStart=performance.now();};
  stability.endFrame=function(){
    var now=performance.now();
    var ms=now-stability.frameStart;
  ...
  stability.handleVisibility=function(){
    if(document.hidden){
      stability.hiddenSince=performance.now();
      stability.paused=true;
      updatePanel();
      return;
    }
    if(stability.hiddenSince){
      stability.hiddenSince=0;
      stability.paused=false;
      if(typeof root.lastT==='number')root.lastT=performance.now();
      if(typeof root.flashT==='number')root.flashT=0;
      if(typeof root.hitstopT==='number')root.hitstopT=0;
      updatePanel();
    }
  };
  if(stability.debug){
    ...
    stability.panel.textContent='AB ENGINE 3.1';document.body.appendChild(stability.panel);
  }
